...
def load_dotenv():
    paths = [".env", "scripts/.env", "../.env", os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), ".env")]
    for path in paths:
        if os.path.exists(path):
            try:
                ...
                print(f"[INFO] 已加载本地环境变量配置文件 {path}")
                break
            except:
                pass
...
def detect_accounts():
    global GLOBAL_COOKIES_LIST, GLOBAL_COOKIES_PATH
    accounts = []
    
    # 方案 0: 从本地 cookies.txt 文件读取 (每行一个账号)
    local_txt = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "cookies.txt")
    raw_cookies = ""
    used_local_path = None
    
    for path in ["cookies.txt", local_txt, os.path.join(os.path.dirname(os.path.abspath(__file__)), "cookies.txt")]:
        if os.path.exists(path):
            try:
                with open(path, "r", encoding="utf-8") as f:
                    raw_cookies = f.read().strip()
                if raw_cookies:
                    used_local_path = path
                    break
            except:
                pass
                
    if used_local_path:
        GLOBAL_COOKIES_PATH = used_local_path
        print(f"[INFO] 从本地文件 {used_local_path} 加载账号配置")
    else:
        # 方案 1: 从单个多行环境变量 WEIRDHOST_COOKIES 读取 (每行一个账号)
        raw_cookies = os.environ.get("WEIRDHOST_COOKIES", "").strip()
        
    if raw_cookies:
        raw_lines = raw_cookies.split("\n")
        GLOBAL_COOKIES_LIST = [line.strip() for line in raw_lines if line.strip()]
        for idx, line in enumerate(GLOBAL_COOKIES_LIST):
            config = parse_account_config(line)
            if not config:
                print(f"[WARN] 账号配置第 {idx+1} 行格式错误，跳过")
                continue
            
            remark = config["remark"] or f"账号{idx+1}"
            source_desc = f"本地文件" if GLOBAL_COOKIES_PATH else "WEIRDHOST_COOKIES"
            print(f"[INFO] 检测到 {source_desc} (第 {idx+1} 行): {mask_remark(remark)}")
            
            accounts.append({
                "index": idx,
                "cookie_env": f"WEIRDHOST_COOKIES_LINE_{idx}",
                "remark": remark,
    ...
    # 方案 2: 若单变量为空，回退到 WEIRDHOST_COOKIE_1 ~ 5
    if not accounts:
        for i in range(1, MAX_COOKIE_COUNT + 1):
            ...
            if not remark:
                remark = os.environ.get(f"WEIRDHOST_REMARK_{i}", "").strip()
            remark = remark or f"账号{i}"
        ...
        for cookie in cookies:
            if cookie.get("name", "").startswith("remember_web"):
                ...
                if new_val and new_val != original_cookie_value:
                    ...
                    if cookie_env.startswith("WEIRDHOST_COOKIES_LINE_"):
                        try:
                            idx = int(cookie_env.split("WEIRDHOST_COOKIES_LINE_")[1])
                            GLOBAL_COOKIES_LIST[idx] = new_secret_value
                            new_multiline = "\n".join(GLOBAL_COOKIES_LIST)
                            
                            # 更新本地 cookies.txt
                            if GLOBAL_COOKIES_PATH and os.path.exists(GLOBAL_COOKIES_PATH):
                                print(f"[INFO]   Cookie 已变化，正在更新本地文件 {GLOBAL_COOKIES_PATH} 的第 {idx+1} 行...")
                                try:
                                    with open(GLOBAL_COOKIES_PATH, "w", encoding="utf-8") as f:
                                        f.write(new_multiline + "\n")
                                    print(f"[INFO]   ✅ 本地 cookies.txt 已自动保存更新")
                                except Exception as e:
                                    print(f"[ERROR]  ❌ 本地 cookies.txt 保存失败: {e}")
                            
                            # 更新 GitHub Secret
                            print(f"[INFO]   尝试更新 GitHub Secret [WEIRDHOST_COOKIES]...")
                            if asyncio.run(update_github_secret("WEIRDHOST_COOKIES", new_multiline)):
                                os.environ["WEIRDHOST_COOKIES"] = new_multiline
                                print(f"[INFO]   ✅ GitHub Secrets [WEIRDHOST_COOKIES] 已更新")
                                return True
                            else:
                                print(f"[WARN]   GitHub Secrets 更新跳过或未配置 (本地运行通常会跳过)")
                                return bool(GLOBAL_COOKIES_PATH) # 如果本地保存成功，返回 True
                        except Exception as ex:
                            print(f"[ERROR]  更新多行变量异常: {ex}")
                            return False
                    else:
